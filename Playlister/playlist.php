...
$newPath = set_include_path(get_include_path() . PATH_SEPARATOR . $_SERVER['DOCUMENT_ROOT']."/ZendGdata-1.12.3/library");
...
echo "<center><h2><u>Youtube Playlister</u></h2><br></center>";

$apikey = "AIzaSyCQoRs6fQ00x6-cSO1VAqPkHWoqZPNR6X4";
$httpClient = getAuthSubHttpClient();
if($httpClient==null) die();

try {
	$yt = new Zend_Gdata_YouTube($httpClient, "Playlister", null, $apikey);
	echo "11";die;
	print_r($yt);die;
...
if($Playlist===null){
	...
	$Playlist->summary = $yt->newSummary()->setText('Playlist');
...
if(isset($_SESSION['POST']['Add2Playlist'])){
  ...
  foreach(array_keys($POST) as $post){
	...
	try { 
	  ...
	  // post
	  $yt->insertEntry($newPlaylistEntry, $postUrl); 
...
{
    $next = 'http://localhost/youtube_playlist/playlist.php';
...
{
echo $_SESSION['sessionToken']."==".$_GET['token'];
...
{
  global $yt; //$yt = new Zend_Gdata_YouTube();
